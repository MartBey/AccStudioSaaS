...
export class DomainManager {
  ...
  analyzeDomain(req: NextRequest): DomainInfo {
    ...
    const isDev = process.env.NODE_ENV === 'development' || hostname.includes('localhost') || hostname.includes('.local');
  ...
  isStaticFile(pathname: string): boolean {
    const staticExtensions = ['.css', '.js', '.png', '.jpg', '.svg', '.ico', '.json', '.txt', '.xml', '.webp', '.avif', '.mp4', '.webm', '.pdf'];
    const staticPaths = ['/_next', '/assets', '/images', '/fonts', '/uploads'];
    return staticPaths.some(p => pathname.startsWith(p)) || staticExtensions.some(ext => pathname.includes(ext));
  }
