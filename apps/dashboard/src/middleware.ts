...
async function getSiteByDomain(
  hostname: string
): Promise<{ siteId: string; subdomain?: string } | null> {
  // 1. Check cache first
  ...
  try {
    ...
    const url = new URL(
      `${process.env.AUTH_URL || "http://localhost:3000"}/api/sites/by-domain?hostname=${hostname}`
    );
