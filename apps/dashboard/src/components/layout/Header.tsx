...
export function Header({ items }: HeaderProps) {
  ...
  const currentItem = items.find(i => pathname === i.href || (i.href !== "/" && pathname?.startsWith(i.href)));
  ...
  const rolePrefix = pathname?.startsWith('/marka') ? '/marka' : pathname?.startsWith('/ajans') ? '/ajans' : '/freelancer';
      ...
      <div className="flex items-center gap-4">
        <Sheet>
          ...
          <SheetContent side="left" className="w-64 p-0">
            <div className="p-6">
              <h2 className="font-bold text-2xl text-primary mb-6">AccStudio</h2>
                  ...
                  return (
                    <Link
                      ...
                      className={cn(
                        ...
                        isActive ? "bg-primary/10 text-primary" : "text-muted-foreground hover:bg-muted hover:text-primary"
                      )}
                    >
          ...
          <TooltipProvider>
            <Tooltip delayDuration={300}>
              <TooltipTrigger asChild>
                <div className="flex items-center gap-1.5 text-sm font-medium text-muted-foreground hover:text-foreground cursor-help transition-colors border rounded-full px-3 py-1.5 bg-background shadow-sm mr-2 hidden sm:flex">
                  <LucideIcons.Star className="h-4 w-4" />
                  <span>Sıralamam: 524</span>
                  <LucideIcons.Info className="h-4 w-4 ml-1 text-muted-foreground/70" />
                </div>
              </TooltipTrigger>
              <TooltipContent className="max-w-[280px] p-3 shadow-lg" sideOffset={8}>
                <p className="text-sm font-medium leading-relaxed">
                  Burada diğer hizmet verenlere kıyasla performansını görebilirsin. İş kazanmak ve sıralamanı yükseltmek için teklif ver!
                </p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
          ...
          <DropdownMenuContent align="end" className="w-56">
            <DropdownMenuLabel className="font-normal">
              <div className="flex flex-col space-y-1">
                <p className="text-sm font-medium leading-none">{session?.user?.name || session?.user?.email || "Kullanıcı"}</p>
