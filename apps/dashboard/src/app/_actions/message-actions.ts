...
export async function getMessageThreads() {
    ...
    include: {
      profile: {
        include: {
          ...
          freelancer: {
            include: { tasks: { include: { project: { select: { id: true, name: true } } } } },
          },
  ...
  // Brand projeleri
  user?.profile?.brand?.projects.forEach((p) => {
    projectIds.add(p.id);
    projectNames.set(p.id, p.name);
  });
  // Agency projeleri
  user?.profile?.agency?.projects.forEach((p) => {
    projectIds.add(p.id);
    projectNames.set(p.id, p.name);
  });
  // Freelancer görevlerinin projeleri
  user?.profile?.freelancer?.tasks.forEach((t) => {
    projectIds.add(t.project.id);
    projectNames.set(t.project.id, t.project.name);
  });
  ...
  return threads
    .filter((t) => t.totalMessages > 0 || true)
    .sort((a, b) => {
...
export async function sendMessage(data: { projectId: string; content: string }) {
  ...
  try {
    ...
    try {
      const { getPusherServer, getProjectChannel, PUSHER_EVENTS } =
        await import("@/lib/pusher-server");
      const pusher = getPusherServer();
      await pusher.trigger(getProjectChannel(data.projectId), PUSHER_EVENTS.NEW_MESSAGE, {
        id: message.id,
