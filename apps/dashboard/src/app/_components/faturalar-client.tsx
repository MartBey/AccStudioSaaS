...
import { CreditCard, DollarSign, Clock, CheckCircle2, XCircle, TrendingUp } from "lucide-react";
...
const statusMap: Record<string, { label: string; color: string; icon: React.ReactNode }> = {
  PENDING: { label: "Bekliyor", color: "bg-yellow-100 text-yellow-700", icon: <Clock className="h-3.5 w-3.5" /> },
  PAID: { label: "Ödendi", color: "bg-emerald-100 text-emerald-700", icon: <CheckCircle2 className="h-3.5 w-3.5" /> },
  CANCELLED: { label: "İptal", color: "bg-red-100 text-red-700", icon: <XCircle className="h-3.5 w-3.5" /> },
  REFUNDED: { label: "İade", color: "bg-blue-100 text-blue-700", icon: <XCircle className="h-3.5 w-3.5" /> },
};

export default function FaturalarClient({ payments, totalPaid, totalPending, total, role }: FaturalarClientProps) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card className="bg-gradient-to-br from-emerald-500/10 to-emerald-600/5">
          <CardContent className="py-5 text-center">
            ...
            <p className="text-2xl font-bold text-emerald-700">₺{totalPaid.toLocaleString("tr-TR")}</p>
            <p className="text-xs text-muted-foreground">{isBrand ? "Toplam Ödenen" : "Tamamlanan Kazanç"}</p>
          </CardContent>
        </Card>
        <Card className="bg-gradient-to-br from-yellow-500/10 to-yellow-600/5">
          <CardContent className="py-5 text-center">
            <Clock className="h-6 w-6 text-yellow-600 mx-auto mb-1" />
            <p className="text-2xl font-bold text-yellow-700">₺{totalPending.toLocaleString("tr-TR")}</p>
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-3">
              {payments.map(p => (
                <div key={p.id} className="flex items-center justify-between p-4 rounded-lg border hover:shadow-sm transition-shadow">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {isBrand && p.status === "PENDING" && (
                      <div className="flex gap-1">
                        <Button size="sm" className="h-7 text-xs" onClick={() => handleStatusUpdate(p.id, "PAID")} disabled={isPending}>
                          Öde
                        </Button>
                        <Button size="sm" variant="outline" className="h-7 text-xs text-red-600" onClick={() => handleStatusUpdate(p.id, "CANCELLED")} disabled={isPending}>
                          İptal
                        </Button>
                      </div>
                    )}
