...
import { Activity, FolderOpen, Plus, TrendingUp, Users } from "lucide-react";
...
export default function MarkaDashboardClient({
  brandName,
  stats,
  recentProjects,
}: MarkaDashboardClientProps) {
  return (
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-7">
        <Card className="col-span-4 flex flex-col p-6">
          <div className="mb-4 flex items-center justify-between">
            <h3 className="text-lg font-semibold">Son Projeler</h3>
            <Link href="/marka/projeler" className="text-sm text-primary hover:underline">
              Tüm Projeler
            </Link>
          </div>
          <div className="flex-1 space-y-4">
            {recentProjects.length === 0 ? (
              <p className="py-4 text-sm text-muted-foreground">Henüz proje oluşturmadınız.</p>
            ) : (
          ...
          <div className="flex-1 space-y-3">
            <Link href="/marka/projeler" className="block">
              <div className="rounded-lg bg-muted/50 p-3 transition-colors hover:bg-muted">
                ...
                <p className="mt-1 text-xs text-muted-foreground">Proje oluşturun ve yönetin</p>
              </div>
            </Link>
            <Link href="/marka/seo-analiz" className="block">
              <div className="rounded-lg bg-muted/50 p-3 transition-colors hover:bg-muted">
                <span className="text-sm font-medium">🔎 SEO Analiz</span>
                <p className="mt-1 text-xs text-muted-foreground">Sitenizi analiz edin</p>
              </div>
            </Link>
            <Link href="/marka/ai-icerik" className="block">
              <div className="rounded-lg bg-muted/50 p-3 transition-colors hover:bg-muted">
                <span className="text-sm font-medium">🤖 AI İçerik</span>
                <p className="mt-1 text-xs text-muted-foreground">AI ile içerik üretin</p>
              </div>
            </Link>
            <Link href="/marka/sosyal-medya" className="block">
              <div className="rounded-lg bg-muted/50 p-3 transition-colors hover:bg-muted">
                <span className="text-sm font-medium">📱 Sosyal Medya</span>
                <p className="mt-1 text-xs text-muted-foreground">Paylaşımlarınızı yönetin</p>
              </div>
            </Link>
          </div>
        </Card>
