      ...
      <div className="flex items-center gap-1.5 overflow-x-auto border-b border-border pb-3">
        {TABS.map((t) => {
          ...
          return (
            ...
            >
              {TAB_LABELS[t]} <span className="ml-1 opacity-60">({count})</span>
            </button>
          );
        })}
      </div>
            ...
            return (
              <div
                ...
                className={cn(
                  ...
                  isOpen ? "border-red-500/30" : "border-border"
                )}
              >
                {/* Header */}
                <div
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex min-w-0 items-start gap-3">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="line-clamp-2 text-sm">{d.reason}</p>
                        <div className="mt-1.5 flex items-center gap-3 text-[10px] text-muted-foreground">
                          <span className="flex items-center gap-1">
                            <User className="h-3 w-3" />
                            {d.raisedBy.name || d.raisedBy.email} ({d.raisedBy.role})
                          </span>
                          <span className="flex items-center gap-1">
                  ...
                  <div className="space-y-4 border-t border-border bg-muted/10 p-4">
                          ...
                          <div className="rounded-md border border-border p-3">
                            ...
                            {d.project.budget && (
                              <p className="text-xs text-muted-foreground">
                                Bütçe: ₺{d.project.budget.toLocaleString("tr-TR")}
                              </p>
                            )}
                          </div>
                        )}
                    ...
                    {d.resolution && (
                      <div className="rounded-md border border-emerald-500/20 bg-emerald-500/5 p-3">
                        <p className="mb-1 text-xs font-medium text-emerald-400">Karar</p>
                        <p className="text-sm">{d.resolution}</p>
                        {d.resolvedBy && (
                          <p className="mt-1 text-[10px] text-muted-foreground">
                            — {d.resolvedBy.name},{" "}
                            {d.resolvedAt && new Date(d.resolvedAt).toLocaleDateString("tr-TR")}
                          </p>
                        )}
                      </div>
                    )}
                      ...
                      <div className="space-y-3">
                        {d.status === "OPEN" && (
                          <button
                            ...
                            disabled={isPending}
                            className="flex items-center gap-1.5 rounded-md bg-yellow-500/10 px-3 py-1.5 text-xs font-medium text-yellow-400 ring-1 ring-yellow-500/20 transition-colors hover:bg-yellow-500/20 disabled:opacity-50"
                          >
                        ...
                        <div>
                          ...
                          <textarea
                            ...
                            rows={3}
                            className="w-full resize-none rounded-md border border-border bg-background p-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/40"
                          />
                        </div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          <button
                            ...
                            className="flex items-center gap-1.5 rounded-md bg-purple-500/10 px-3 py-1.5 text-xs font-medium text-purple-400 ring-1 ring-purple-500/20 transition-colors hover:bg-purple-500/20 disabled:opacity-50"
                          >
                          ...
                          <button
                            ...
                            disabled={isPending || !resolution.trim()}
                            className="flex items-center gap-1.5 rounded-md bg-muted px-3 py-1.5 text-xs font-medium text-muted-foreground ring-1 ring-border transition-colors hover:bg-muted/80 disabled:opacity-50"
                          >
                            Kapat (Geçersiz)
                          </button>
                        </div>
