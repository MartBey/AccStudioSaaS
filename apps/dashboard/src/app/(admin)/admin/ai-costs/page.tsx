...
import {
  Brain, TrendingUp, Zap, AlertTriangle,
          ...
          {data.actionBreakdown.length === 0 ? (
            ...
            <div className="divide-y divide-border/50">
              {data.actionBreakdown.map((a) => {
                ...
                return (
                  <div key={a.action} className="px-4 py-3 flex items-center gap-3">
                    <span
                      className={cn(
                ...
                <div key={i} className="px-4 py-3 flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-muted-foreground">
                      {u.requestCount} istek · {(u.totalInputTokens + u.totalOutputTokens).toLocaleString("tr-TR")} token
                    </p>
                  </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-muted/20">
                <th className="text-left px-4 py-2.5 font-medium text-muted-foreground text-xs">Kullanıcı</th>
                <th className="text-left px-4 py-2.5 font-medium text-muted-foreground text-xs">Aksiyon</th>
                <th className="text-left px-4 py-2.5 font-medium text-muted-foreground text-xs">Model</th>
                <th className="text-left px-4 py-2.5 font-medium text-muted-foreground text-xs">Token</th>
                <th className="text-left px-4 py-2.5 font-medium text-muted-foreground text-xs">Maliyet</th>
                <th className="text-left px-4 py-2.5 font-medium text-muted-foreground text-xs">Süre</th>
              ...
              {data.recentLogs.length === 0 ? (
                ...
                data.recentLogs.map((l) => (
                  <tr key={l.id} className="border-b border-border/50 hover:bg-muted/20 transition-colors">
                    ...
                    <td className="px-4 py-2.5 text-xs text-muted-foreground font-mono">{l.model}</td>
                    <td className="px-4 py-2.5 text-sm tabular-nums">{l.totalTokens.toLocaleString("tr-TR")}</td>
                    <td className="px-4 py-2.5 text-sm font-semibold tabular-nums">${l.cost.toFixed(6)}</td>
