...
export const BuilderNodePropsSchema = z
  .record(z.any())
  .describe("Blok için gerekli içerik ve stil özellikleri (örn: title, subtitle, color, src)");
...
export const BuilderNodeSchema: z.ZodType<BuilderNode> = z.lazy(() =>
  z.object({
    id: z.string().describe("Bu düğüm için benzersiz bir ID (UUID vb. olabilir, AI uydurabilir)"),
    type: z.nativeEnum(BlockType).describe("Kullanılacak bileşen türü (Örn: Hero, Features)"),
    props: BuilderNodePropsSchema,
    children: z
      .array(BuilderNodeSchema)
      .optional()
      .describe("Bu bileşenin içine eklenecek alt bileşenler (Eğer parent-child destekliyorsa)"),
  })
);
...
export const ThemeConfigSchema = z.object({
  ...
  fontFamily: z.string().describe("Kullanılacak Google Font ailesi (Örn: 'Inter', 'Roboto')"),
  borderRadius: z.string().describe("Genel kenar yuvarlaklığı (Örn: '0.5rem', '8px')").optional(),
});
...
export const BuilderSiteSchema = z.object({
  themeConfig: ThemeConfigSchema.optional().describe("Site geneli tema ayarları"),
  nodes: z
    .array(BuilderNodeSchema)
    .describe("Sitede yer alacak kök(root) düğümlerin listesi (Yukarıdan aşağıya sıralı)"),
});
